...
const EFFECT_MAP = {
  ...
  // ── Sub Combo ────────────────────────────────────────────────────────────────
  'sub.combo': (e) => [
    { effect: 'combo-display', payload: { ...e.payload, sound: 'alert.wav' } },
    ...(e.payload.level >= 3
  ...
  // ── Hype Train ───────────────────────────────────────────────────────────────
  'hype-train.start': () => [
    { effect: 'alert-banner', payload: { tier: 'A', icon: '🔥', text: 'HYPE TRAIN STARTED!', subText: "Let's gooo!", bannerColor: '#FF4500', glowColor: 'rgba(255,69,0,0.6)', sound: 'yay.wav' } },
    { effect: 'confetti',     payload: { sound: 'pop.wav' } },
  ],

  'hype-train.progress': (e) => [
    { effect: 'alert-banner',    payload: { tier: 'S', icon: '🔥', text: `HYPE TRAIN  LEVEL ${e.payload.level}!`, sound: 'alert.wav' } },
    { effect: 'crowd-explosion', payload: { sound: 'boom.wav' } },
  ],

  'hype-train.end': (e) => [
    { effect: 'alert-banner', payload: { tier: 'A', icon: '🔥', text: `Hype Train ended at Level ${e.payload.level}!`, subText: 'Thanks for the hype, chat!', sound: 'yay.wav' } },
    { effect: 'confetti',     payload: { sound: 'pop.wav' } },
    { effect: 'firework-salvo', payload: { count: 3, sound: 'boom.wav' } },
  ],
};
